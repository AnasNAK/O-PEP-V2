...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['updateTheme'])) {
    ...
    if (isset($_POST['tagstatus']) && is_array($_POST['tagstatus'])) {
        foreach ($_POST['tagstatus'] as $tagId) {
            $updatestatus = "UPDATE tag SET TagSt = ? WHERE idTag = ?";
            $tagStatus = in_array($tagId, $_POST['tagstatus']) ? 1 : 0;

            $Upstmt = $mysqli->prepare($updatestatus);
            $Upstmt->bind_param('ii', $tagStatus, $tagId);
            $Upstmt->execute();
        }
        ...
        <form action="" method="POST" enctype="multipart/form-data">
            <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" name="ThemeName" value="<?php echo $theme['ThemeName']; ?>">
                ...
                <div>
                    ...
                    <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-white border-dashed rounded-md">
                        <div class="space-y-1 text-center">
                            ...
                            <img src="./assets/images/<?php echo isset($imagePath) ? $imagePath : ''; ?>" alt="Current Image" class="h-24 w-24 mx-auto rounded-md">
                ...
                ?>
                    <ul class="w-48 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                        <li class="w-full border-b border-gray-200 rounded-t-lg dark:border-gray-600">
                            <div class="flex items-center ps-3">
                                <div class="flex items-center ps-4 border border-gray-200 rounded dark:border-gray-700">
                                    <input id="bordered-checkbox-1<?php echo $tag['idTag']; ?>" type="checkbox" value="<?php echo $tag['idTag']; ?>" name=" tagstatus[]" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
